...
def test_review_workflow_lists_and_resolves_low_confidence_items(invoice_dir, temp_db):
    ...
    queue = list_pending(str(temp_db), "demo-user")
    assert len(queue) == 1

    resolved = resolve_item(str(temp_db), queue[0]["review_item_id"], "accepted", "Confirmado")
    ...
    row = connection.execute(
        "SELECT partial_status, statement_status FROM statements WHERE statement_id = ?",
